...
class MerkleTree {
 protected:
  ...
  static constexpr std::size_t nodeCountAtDepth(std::uint64_t maxDepth);
  static constexpr std::size_t nodeCountUpToDepth(std::uint64_t maxDepth);
  static constexpr std::size_t allocationSize(std::uint64_t maxDepth);
 ...
 protected:
  ...
  std::size_t index(std::uint64_t key, std::uint64_t depth) const;
  ...
  void prepareDiff(MerkleTree<BranchingBits, LockStripes>& other,
  ...
  std::pair<std::uint64_t, std::uint64_t> chunkRange(std::size_t chunk, std::uint64_t depth);
