...
bool normalize(std::string& out, irs::string_ref const& type, VPackSlice const properties) {
  if (type.empty()) {
  ...
  return irs::analysis::analyzers::normalize(out, type, irs::text_format::vpack,
...
bool parse_ngram_vpack_config(const irs::string_ref& args,
...
REGISTER_ANALYZER_VPACK(irs::analysis::ngram_token_stream, ngram_vpack_builder,
                        ngram_vpack_normalizer);
}  // namespace ngram_vpack
...
REGISTER_ANALYZER_VPACK(irs::analysis::text_token_stemming_stream,
                        stem_vpack_builder, stem_vpack_normalizer);
}  // namespace stem_vpack
...
REGISTER_ANALYZER_VPACK(irs::analysis::text_token_normalizing_stream,
                        norm_vpack_builder, norm_vpack_normalizer);
}  // namespace norm_vpack
      ...
      case VPackValueType::String:
        ...
        while (string_analyzer->next()) {
          builder.add(arangodb::iresearch::toValuePair(
              irs::ref_cast<char>(string_terms->value())));
        }
        break;
      case VPackValueType::Bool:
        builder.add(arangodb::iresearch::toValuePair(arangodb::basics::StringUtils::encodeBase64(
      ...
      default:
        if (current.isNumber()) {  // there are many "number" types. To adopt all current and future ones just
          ...
          if (!numeric_analyzer) {
            ...
            numeric_terms =
                numeric_analyzer->attributes().get<irs::term_attribute>().get();
            if (ADB_UNLIKELY(!numeric_terms)) {
              auto const message =
                  "failure to retrieve values from arangosearch numeric "
                  "analyzer "
                  "while computing result for function 'TOKENS'";
...
////////////////////////////////////////////////////////////////////////////////
bool equalAnalyzer(arangodb::iresearch::IResearchAnalyzerFeature::AnalyzerPool const& pool,
  ...
  return type == pool.type()  // same type
         && arangodb::basics::VelocyPackHelper::equal(arangodb::iresearch::slice(normalizedProperties),
...
std::shared_ptr<arangodb::LogicalCollection> getAnalyzerCollection(  // get collection
  ...
  try {
    ...
    LOG_TOPIC("00001", WARN, arangodb::iresearch::TOPIC)
        << "failure to find 'ClusterInfo' instance while looking up Analyzer "
           "collection '"
        << arangodb::StaticStrings::AnalyzersCollection << "' in vocbase '"
        << vocbase.name() << "'";
  } catch (arangodb::basics::Exception& e) {
    LOG_TOPIC("00002", WARN, arangodb::iresearch::TOPIC)
        << "caught exception while looking up Analyzer collection '"
        << arangodb::StaticStrings::AnalyzersCollection << "' in vocbase '"
        << vocbase.name() << "': " << e.code() << " " << e.what();
    IR_LOG_EXCEPTION();
...
std::string normalizedAnalyzerName(std::string database,            // database
...
arangodb::Result visitAnalyzers(  // visit analyzers
  ...
  static const auto resultVisitor =
    ...
    if (!slice.isArray()) {
      return arangodb::Result(  // result
          ...
          std::string("failed to parse contents of collection '") +
              arangodb::StaticStrings::AnalyzersCollection + "' in database '" +
              vocbase.name() + " while visiting analyzers");
    }
  ...
  if (arangodb::ServerState::instance()->isDBServer()) {
    ...
    if (!cc) {
      return arangodb::Result(  // result
          TRI_ERROR_INTERNAL,   // code
          std::string("failure to find 'ClusterComm' instance while visiting "
                      "Analyzer collection '") +
              arangodb::StaticStrings::AnalyzersCollection + "' in vocbase '" +
              vocbase.name() + "'");
    }
    ...
    for (auto& request : requests) {
      ...
      if (!request.result.answer) {
        return arangodb::Result(  // result
            TRI_ERROR_INTERNAL,   // code
            std::string("failed to get answer from 'ClusterComm' instance "
                        "while visiting Analyzer collection '") +
      ...
      if (!slice.hasKey("result")) {
        return arangodb::Result(  // result
            TRI_ERROR_INTERNAL,   // code
            std::string("failed to parse result from 'ClusterComm' instance "
                        "while visiting Analyzer collection '") +
                arangodb::StaticStrings::AnalyzersCollection +
                "' in vocbase '" + vocbase.name() + "'");
      }
  ...
  if (arangodb::ServerState::instance()->isClusterRole()) {
    ...
    static const auto queryString = arangodb::aql::QueryString(  // query to execute
        std::string("FOR d IN ") +
        arangodb::StaticStrings::AnalyzersCollection + " RETURN d"  // query
    );
...
void IResearchAnalyzerFeature::AnalyzerPool::toVelocyPack(VPackBuilder& builder,
  ...
  arangodb::iresearch::addStringRef(builder, StaticStrings::AnalyzerNameField,
                                    forPersistence ? splitAnalyzerName(name()).second
                                                   : irs::string_ref(name()));
...
IResearchAnalyzerFeature::AnalyzerPool::Builder::make(irs::string_ref const& type,
  ...
  // for API consistency we only support analyzers configurable via jSON
  return irs::analysis::analyzers::get(type, irs::text_format::vpack,
...
/*static*/ bool IResearchAnalyzerFeature::canUse(  // check permissions
  ...
  return !ctx  // authentication not enabled
         || (ctx->canUseDatabase(vocbase.name(), level)  // can use vocbase
             && (ctx->canUseCollection(vocbase.name(), arangodb::StaticStrings::AnalyzersCollection,
...
/*static*/ bool IResearchAnalyzerFeature::canUse(  // check permissions
  ...
  if (staticAnalyzers.find(irs::make_hashed_ref(name, std::hash<irs::string_ref>())) !=
      staticAnalyzers.end()) {
  ...
  return split.first.null()  // static analyzer (always allowed)
         || (ctx->canUseDatabase(split.first, level)  // can use vocbase
             && ctx->canUseCollection(split.first, arangodb::StaticStrings::AnalyzersCollection,
                                      level)  // can use analyzers
            );
}
...
arangodb::Result IResearchAnalyzerFeature::emplaceAnalyzer(  // emplace
    ...
    Analyzers& analyzers, irs::string_ref const& name, irs::string_ref const& type,
    VPackSlice const properties, irs::flags const& features) {
  ...
  if (!TRI_vocbase_t::IsAllowedName(false, velocypack::StringRef(split.second.c_str(),
                                                                 split.second.size()))) {
    return arangodb::Result(      // result
    ...
    if (&irs::frequency::type() == feature) {
      ...
      if (!features.check(irs::frequency::type())) {
        return arangodb::Result(TRI_ERROR_BAD_PARAMETER,
                                "missing feature '" +
                                    std::string(irs::frequency::type().name()) +
                                    "' required when '" + std::string(feature->name()) +
  ...
  static const auto generator = [](  // key + value generator
    ...
    const_cast<AnalyzerPool::ptr&>(value) =
        pool;  // lazy-instantiate pool to avoid allocation if pool is already present
  ...
  try {
    ...
    if (itr.second) {
      if (!allowCreation) {
        return arangodb::Result(      // result
            TRI_ERROR_BAD_PARAMETER,  // code
            "forbidden implicit creation of an arangosearch analyzer instance "
            "for name '" +
                std::string(name) + "' type '" + std::string(type) +
                "' properties '" + properties.toString() + "'");
      }
...
IResearchAnalyzerFeature::AnalyzerPool::ptr IResearchAnalyzerFeature::get(  // find analyzer
  ...
  try {
      ...
      if (!onlyCached) {
        ...
        if (!res.ok()) {
          LOG_TOPIC("36062", WARN, arangodb::iresearch::TOPIC)
              << "failure to load analyzers for database '" << split.first
    ...
    SCOPED_LOCK(mutex);
    auto itr = _analyzers.find(irs::make_hashed_ref(static_cast<irs::string_ref>(normalizedName),
    ...
    Instance() {
      // register the indentity analyzer
      {
        ...
        if (!pool || !pool->init(IdentityAnalyzer::type().name(),
                                 VPackSlice::emptyObjectSlice(), extraFeatures)) {
        ...
        analyzers.emplace(irs::make_hashed_ref(irs::string_ref(pool->name()),
                                               std::hash<irs::string_ref>()),
                          pool);
      }
      ...
      // register the text analyzers
      {
        ...
        std::vector<irs::string_ref> const locales = {"de", "en", "es", "fi",
                                                      "fr", "it", "nl", "no",
                                                      "pt", "ru", "sv", "zh"};

        irs::flags const extraFeatures = {irs::frequency::type(),
        ...
        for (auto const& locale : locales) {
          ...
          // { locale: "<locale>.UTF-8", stopwords: [] }
          {
            ...
            properties.add("locale",
                           VPackValue(std::string(locale) + ".UTF-8"));
...
/*static*/ IResearchAnalyzerFeature::AnalyzerPool::ptr IResearchAnalyzerFeature::identity() noexcept {
  struct Identity {
    ...
    Identity() {
      ...
      auto& staticAnalyzers = getStaticAnalyzers();
      irs::string_ref name =
          IDENTITY_ANALYZER_NAME;  // hardcoded name of the identity analyzer pool
...
arangodb::Result IResearchAnalyzerFeature::loadAnalyzers(irs::string_ref const& database /*= irs::string_ref::NIL*/) {
  try {
    ...
    static auto cleanupAnalyzers = [](  // remove invalid analyzers
      ...
      // remove no longer valid analyzers (force remove)
      for (auto itr = feature._analyzers.begin(), end = feature._analyzers.end();
           itr != end;) {
        auto split = splitAnalyzerName(itr->first);
    ...
    if (!vocbase) {
      ...
      return arangodb::Result(                  // result
          ...
          std::string("failed to find database '") + std::string(database) +
    ...
    Analyzers analyzers;
    auto visitor = [this, &analyzers, &vocbase](
                       arangodb::velocypack::Slice const& slice) -> arangodb::Result {
      if (!slice.isObject()) {
      ...
      if (!slice.hasKey(arangodb::StaticStrings::KeyString)  // no such field (required)
        ...
        LOG_TOPIC("1dc56", ERR, arangodb::iresearch::TOPIC)
            ...
            << arangodb::StaticStrings::KeyString << "' while loading analyzer form collection '"
            << arangodb::StaticStrings::AnalyzersCollection << "' in database '"
            << vocbase->name() << "', skipping it: " << slice.toString();
      ...
      if (!slice.hasKey("name")  // no such field (required)
        ...
        LOG_TOPIC("f5920", ERR, arangodb::iresearch::TOPIC)
            << "failed to find a string value for analyzer 'name' while "
               "loading analyzer form collection '"
            << arangodb::StaticStrings::AnalyzersCollection << "' in database '"
            << vocbase->name() << "', skipping it: " << slice.toString();
      ...
      if (!slice.hasKey("type")  // no such field (required)
        ...
        LOG_TOPIC("9f5c8", ERR, arangodb::iresearch::TOPIC)
            << "failed to find a string value for analyzer 'type' while "
               "loading analyzer form collection '"
            << arangodb::StaticStrings::AnalyzersCollection << "' in database '"
            << vocbase->name() << "', skipping it: " << slice.toString();
      ...
      if (slice.hasKey("features")) {
        ...
        if (!subSlice.isArray()) {
          LOG_TOPIC("7ec8a", ERR, arangodb::iresearch::TOPIC)
              << "failed to find an array value for analyzer 'features' while "
                 "loading analyzer form collection '"
              << arangodb::StaticStrings::AnalyzersCollection << "' in database '"
              << vocbase->name() << "', skipping it: " << slice.toString();
        ...
        for (arangodb::velocypack::ArrayIterator subItr(subSlice); subItr.valid(); ++subItr) {
          ...
          if (!subEntry.isString() && !subSlice.isNull()) {
            LOG_TOPIC("7620d", ERR, arangodb::iresearch::TOPIC)
                << "failed to find a string value for an entry in analyzer "
                   "'features' while loading analyzer form collection '"
                << arangodb::StaticStrings::AnalyzersCollection << "' in database '"
                << vocbase->name() << "', skipping it: " << slice.toString();
          ...
          if (!feature) {
            LOG_TOPIC("4fedc", ERR, arangodb::iresearch::TOPIC)
                << "failed to find feature '" << featureName
                << "' while loading analyzer form collection '"
                << arangodb::StaticStrings::AnalyzersCollection << "' in database '"
                << vocbase->name() << "', skipping it: " << slice.toString();
      ...
      if (split.first != vocbase->name()) {
        ...
        if (!result.second) {       // existing entry
          if (result.first->second  // valid new entry
              && !equalAnalyzer(*(entry.second), result.first->second->type(),
            ...
            return arangodb::Result(      // result
                ...
                "arangosearch analizer name '" +
                    std::string(result.first->second->name()) + "' type '" +
                    std::string(result.first->second->type()) +
          ...
          result.first->second =
              entry.second;  // reuse old analyzer pool to avoid duplicates in memmory
      ...
      if (itr->second  // valid new entry
        ...
        return arangodb::Result(      // result
            ...
            "arangosearch analizer name '" +
                ...
                itr->second->properties().toString() +
    ...
    return arangodb::Result(  // result
        e.code(),             // code
        std::string("caught exception while loading configuration for "
...
/*static*/ irs::string_ref IResearchAnalyzerFeature::extractVocbaseName(irs::string_ref const& name) {  // analyzer name (normalized)
  auto split = splitAnalyzerName(name);
...
/*static*/ std::string IResearchAnalyzerFeature::normalize(  // normalize name
  ...
  if (expandVocbasePrefix) {
    ...
    // .........................................................................
    if (&systemVocbase == &activeVocbase || split.first.null() ||
  ...
  try {
    ...
    if (itr == _analyzers.end()) {
      return arangodb::Result(                  // result
          ...
          std::string("failure to find analyzer while removing arangosearch "
                      "analyzer '") +
              std::string(name) + "'");
    }
    ...
    if (!pool) {
      ...
      return arangodb::Result(                  // result
          TRI_ERROR_ARANGO_DOCUMENT_NOT_FOUND,  // code
          std::string("failure to find a valid analyzer while removing "
    ...
    if (pool->_key.null()) {
      return arangodb::Result(  // result
          TRI_ERROR_INTERNAL,   // code
          std::string("failure to find '") + arangodb::StaticStrings::KeyString +
    ...
    if (engine && engine->inRecovery()) {
      return arangodb::Result(  // result
          TRI_ERROR_INTERNAL,   // code
          std::string("failure to remove arangosearch analyzer '") + std::string(name) +
    ...
    if (!dbFeature) {
      return arangodb::Result(  // result
          TRI_ERROR_INTERNAL,   // code
          std::string("failure to find feature 'Database' while removing "
                      "arangosearch analyzer '") +
              std::string(name) + "'");
    }
    ...
    if (!vocbase) {
      return arangodb::Result(                  // result
          TRI_ERROR_ARANGO_DATABASE_NOT_FOUND,  // code
          std::string("failure to find vocbase while removing arangosearch "
                      "analyzer '") +
              std::string(name) + "'");
    }
    ...
    if (!collection) {
      return arangodb::Result(                     // result
          ...
          std::string("failure to find collection while removing arangosearch "
                      "analyzer '") +
              std::string(name) + "'");
    }
    ...
    arangodb::SingleCollectionTransaction trx(  // transaction
        ...
        arangodb::StaticStrings::AnalyzersCollection,  // collection name
    ...
    return arangodb::Result(  // result
        e.code(),             // code
        std::string("caught exception while removing configuration for "
...
arangodb::Result IResearchAnalyzerFeature::storeAnalyzer(AnalyzerPool& pool) {
  ...
  if (!dbFeature) {
    return arangodb::Result(  // result
        TRI_ERROR_INTERNAL,   // code
        std::string("failure to find feature 'Database' while persising "
                    "arangosearch analyzer '") +
            pool.name() + "'");
  }
  ...
  if (pool.type().null()) {
    return arangodb::Result(      // result
        TRI_ERROR_BAD_PARAMETER,  // code
        std::string("failure to persist arangosearch analyzer '") +
  ...
  if (engine && engine->inRecovery()) {
    return arangodb::Result(  // result
        TRI_ERROR_INTERNAL,   // code
        std::string("failure to persist arangosearch analyzer '") +
            pool.name() + "' configuration while storage engine in recovery");
  }
  ...
  try {
    ...
    if (!collection) {
      return arangodb::Result(                     // result
          TRI_ERROR_ARANGO_DATA_SOURCE_NOT_FOUND,  // code
          std::string("failure to find collection '") + arangodb::StaticStrings::AnalyzersCollection +
              "' in vocbase '" + vocbase->name() +
              "' vocbase while persising arangosearch analyzer '" +
              pool.name() + "'");
    }
    ...
    return arangodb::Result(  // result
        e.code(),             // code
        std::string("caught exception while persisting configuration for "
                    "arangosearch analyzer name '") +
...
bool IResearchAnalyzerFeature::visit(  // visit analyzers
  ...
  if (!res.ok()) {
    LOG_TOPIC("73695", WARN, arangodb::iresearch::TOPIC)
        << "failure to load analyzers while visiting database '" << vocbase->name()
        << "': " << res.errorNumber() << " " << res.errorMessage();
    TRI_set_errno(res.errorNumber());
