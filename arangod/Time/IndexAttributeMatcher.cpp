...
#include "IndexAttributeMatcher.h"
...
bool IndexAttributeMatcher::accessFitsIndex(
    ...
    std::unordered_map<size_t /*offset in idx->fields()*/,
    std::vector<arangodb::aql::AstNode const*> /*conjunct - operation*/>& found,  // marks operations covered by index-fields
  ...
  bool const isPrimaryIndex = false;//idx->type() == arangodb::Index::IndexType::TRI_IDX_TYPE_PRIMARY_INDEX;
  ...
  TRI_ASSERT(idx->fields().size() >= 1 && idx->fields()[0].size() == 1);
  TRI_ASSERT(idx->fields()[0][0].name == StaticStrings::TimeString);
  ...
  for (size_t i = 0; i < idx->fields().size(); ++i) {
    ...
    if (i == 0 && op->type == arangodb::aql::NODE_TYPE_OPERATOR_BINARY_IN) {
      // do not support IN on _time field
      continue;
    }
    
    bool isRangeCmp = op->type != aql::NODE_TYPE_OPERATOR_BINARY_EQ &&
                      op->type != aql::NODE_TYPE_OPERATOR_BINARY_NE;
    if (i > 0 && isRangeCmp) { // disallow <, <=, >, >= on fields other than _time
      continue;
    }
...
void IndexAttributeMatcher::matchAttributes(
  ...
  for (size_t i = 0; i < node->numMembers(); ++i) {
    ...
    switch (op->type) { // FIXME can we allow NE ??
...
Index::FilterCosts IndexAttributeMatcher::supportsFilterCondition(
...
Index::SortCosts IndexAttributeMatcher::supportsSortCondition(
    arangodb::Index const* idx, arangodb::aql::SortCondition const* sortCondition,
    arangodb::aql::Variable const* reference, size_t itemsInIndex) {
  TRI_ASSERT(sortCondition != nullptr);
  TRI_ASSERT(idx->type() == Index::TRI_IDX_TYPE_TIMESERIES);
  TRI_ASSERT(!idx->sparse());
...
arangodb::aql::AstNode* IndexAttributeMatcher::specializeCondition(
...
bool IndexAttributeMatcher::isDuplicateOperator(arangodb::aql::AstNode const* node,
