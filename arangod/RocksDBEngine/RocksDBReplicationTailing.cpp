...
class WALParser final : public rocksdb::WriteBatch::Handler {
 ...
 public:
  ...
  void LogData(rocksdb::Slice const& blob) override {
    ...
    LOG_TOPIC(_LOG, Logger::ENGINES) << "[LOG] " << rocksDBLogTypeName(type);
    switch (type) {
  ...
  void writeCommitMarker() {
    ...
    LOG_TOPIC(_LOG, Logger::ENGINES) << "tick: " << _currentSequence << " commit transaction";
 ...
 private:
  ...
  LogicalCollection* loadCollection(TRI_voc_cid_t cid) {
    ...
    if (_vocbase != nullptr) {
      ...
      if (collection) {
        _collectionCache.emplace(cid, CollectionGuard(_vocbase, collection, __FILE__, __LINE__));
...
RocksDBReplicationResult rocksutils::tailWal(TRI_vocbase_t* vocbase, uint64_t tickStart,
  ...
  while (iterator->Valid() && lastTick <= tickEnd && builder.buffer()->size() < chunkSize) {
    ...
    LOG_TOPIC(_LOG, Logger::ENGINES) << "Start WriteBatch tick: " << lastTick;
    ...
    LOG_TOPIC(_LOG, Logger::ENGINES) << "End WriteBatch written-tick: " << lastWrittenTick;
  ...
  // LOG_TOPIC(WARN, Logger::ENGINES) << "2.  lastWrittenTick: " << lastWrittenTick;
  return result;
}
