...
class DependencyProxy {
 public:
  ...
  TEST_VIRTUAL std::pair<ExecutionState, size_t> skipSomeForDependency(
      size_t dependency, size_t atMost, size_t subqueryDepth);

  // TODO enable_if<allowBlockPassthrough>
  std::pair<ExecutionState, SharedAqlItemBlockPtr> fetchBlockForPassthrough(size_t atMost);

  TEST_VIRTUAL std::pair<ExecutionState, size_t> skipSome(size_t atMost, size_t subqueryDepth);
