...
Scheduler::Scheduler(uint64_t nrMinimum, uint64_t nrMaximum,
                     uint64_t maxQueuedHighPrio, uint64_t fifo1Size,
                     uint64_t fifo2Size)
    : _maxQueuedLowPrio(std::max(static_cast<uint64_t>(1), nrMaximum - nrMinimum)),
      _maxQueuedHighPrio(maxQueuedHighPrio),
      _counters(0),
...
void Scheduler::post(std::function<void()> const callback) {
  ...
  _ioContext->post([this, callback]() {
    // start working (and reduce the queue afterwards, we might post
    // less work)
...
void Scheduler::addQueueStatistics(velocypack::Builder& b) const {
  ...
  b.add("queue-size-high-prio", VPackValue(_maxQueuedHighPrio));
  b.add("queue-size-low-prio", VPackValue(_maxQueuedLowPrio));
...
std::string Scheduler::infoStatus() {
  ...
  return "scheduler " + std::to_string(numRunning(counters)) + " (" +
         ...
         ") in-progress " + std::to_string(numWorking(counters)) + " queued high prio " +
         std::to_string(numQueued(counters)) +
         " (<=" + std::to_string(_maxQueuedHighPrio) + ") queued low prio limit " +
         std::to_string(_maxQueuedLowPrio) + " F1 " + std::to_string(_fifoSize[FIFO1]) +
...
bool Scheduler::start() {
  ...
  TRI_ASSERT(0 < _maxQueuedHighPrio);
  TRI_ASSERT(0 < _maxQueuedLowPrio);
