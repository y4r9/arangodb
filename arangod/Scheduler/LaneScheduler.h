...
#ifndef ARANGOD_SCHEDULER_LANE_SCHEDULER_H
#define ARANGOD_SCHEDULER_LANE_SCHEDULER_H 1
...
class LaneScheduler final : public Scheduler {
  LaneScheduler(LaneScheduler const&) = delete;
  LaneScheduler& operator=(LaneScheduler const&) = delete;

  friend class arangodb::JobGuard;

 public:
  LaneScheduler(uint64_t minThreads, uint64_t maxThreads, uint64_t fifo1Size,
                uint64_t fifo2Size);
  virtual ~LaneScheduler();
 ...
 protected:
  bool isStopping() const noexcept override {
    return (_counters & (1ULL << 63)) != 0;
  }
 ...
 private:
  ...
  bool canPostDirectly(RequestLane, RequestPriority, bool allowDirectHandling) const noexcept;
  ...
  bool pushToFifo(int64_t fifo, std::function<void()> callback);
