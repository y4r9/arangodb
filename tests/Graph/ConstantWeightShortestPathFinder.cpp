...
TEST_CASE("ConstantWeightShortestPathFinder", "[graph]") {
  ...
  auto checkPath = [spo](ShortestPathResult result, std::vector<std::string> vertices,
    ...
    CHECK((result.length() == vertices.size()));
    for (size_t i = 0; i < result.length(); i++) {
      auto vert = result.vertexToAqlValue(spo->cache(), i);
      CHECK(vert.slice().get(StaticStrings::KeyString).isEqualString(vertices.at(i)));
    }
    ...
    for (size_t i = 1; i < result.length(); i++) {
      ...
      CHECK(
          edge.slice().get(StaticStrings::FromString).isEqualString(edges.at(i).first));
      CHECK(edge.slice().get(StaticStrings::ToString).isEqualString(edges.at(i).second));
    }
    return true;
  };
  ...
  SECTION("two paths of length 5") {
...

    {
      ...
      REQUIRE(rr);
      REQUIRE(checkPath(result, {"21", "22", "23", "24", "25"},
                        {{},
                         {"v/21", "v/22"},
                         {"v/22", "v/23"},
                         {"v/23", "v/24"},
